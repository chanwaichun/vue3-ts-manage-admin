...
export const useTable = (
	...
	const getTableList = async () => {
		try {
			...
			isPageable && updatePageable({ pageNum: current, pageSize: size, total });
			// 先写着逻辑

			nextTick(() => {
				if (tableRef) {
					// console.log(tableRef.value);
					selectedList.value.forEach((listItem: any) => {
						tableRef.value.toggleRowSelection(toRaw(listItem), true);
					});
				}
			});
		} catch (error) {
