<template>
	<div class="table-box orderDetail">
		...
		<template v-if="!loading">
			<div class="orderDetail-title">当前订单状态：{{ filterEnum(detail.status, payStatus) }}</div>
			<div class="orderDetail-body">
				...
				<div class="productInfo body-item">
					支付信息
					<div :class="{ empty: detail.payRecordVoList?.length === 0 }" class="orderTable">
						<NormalTable :columns="columnPay" :data="detail.payRecordVoList"></NormalTable>
					</div>
				</div>
				<div class="productInfo body-item">
					商品信息
...
<script lang="tsx" name="productEdit" setup>
...
import { setImageUrl, setProtocolUrl } from "@/utils/format";
import { payStatus, payMethods } from "@/utils/serviceDict";
...
onMounted(async () => {
	...
	loading.value = true;
	const res: any = await getOrderDetail(route.query);
...
const columnProduct: ColumnProps<any>[] = [
	{
		...
		render: scope => {
			return (
				<el-image style="width: 200px; height: 200px" src={setImageUrl(scope.row.img)} fit="scale-down" />
				// <div class="image">
				// 	<img class={"userAvatar"} src={setImageUrl(scope.row.img)} style={{ width: "200px" }}></img>;
				// </div>
			);
		}
	},
